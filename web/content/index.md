...
screenshot_alt: "@sebastienrousseau/docker-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/docker-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/docker-config — Shareable Docker and container configuration standards for hardened, multi-stage production builds."
description: "Shareable Docker and container configuration standards for hardened, multi-stage production builds."
keywords: "docker-config, docker, containerization, dockerfile, multi-stage build, container security, devops, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.3"
hero_tagline: "Hardened container configuration and multi-stage build standards"
headline: "@sebastienrousseau/docker-config"
lead: "Shareable Docker and container configuration standards for hardened, multi-stage production builds."
cta_primary: "Install @sebastienrousseau/docker-config"
...
snippet_caption: "Install @sebastienrousseau/docker-config"
snippet_code: "npm install --save-dev @sebastienrousseau/docker-config"
...
## What is @sebastienrousseau/docker-config?

**@sebastienrousseau/docker-config** is an enterprise-grade, shareable configuration for [Docker](https://github.com/sebastienrousseau/docker-config)—enterprise container standards and multi-stage Docker build templates for Node.js and TypeScript services. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.
...
## Why Developers Choose @sebastienrousseau/docker-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/docker-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/docker-config` into any new or existing repository requires only three simple steps.
...
Create or update your `Dockerfile` file at the root of your project to extend the shared configuration:

```dockerfile
# Production standards from @sebastienrousseau/docker-config
...
### Step 3: Run Docker in Your Workflow

Execute Docker directly or configure standard quality scripts in your `package.json`:

```bash
docker build -t app:latest .
```

Compile minimal, non-root, unprivileged OCI container images.
...
### What security optimizations are included?
Configurations enforce non-root user execution, multi-stage build caching, minimal scratch/alpine base images, and exclusion of sensitive local files via standard .dockerignore rules.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `Dockerfile` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/docker-config safe for production CI/CD?
