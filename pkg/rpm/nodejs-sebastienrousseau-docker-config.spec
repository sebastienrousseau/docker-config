...
%global npm_bare      docker-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-docker-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Docker container configuration providing multi-stage build and security rules

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/docker-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/docker-config is a shareable configuration preset with no runtime
...
install -pm 0644 Dockerfile %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
